...
int serverShutdownFlag = 0;        // global server shutdown flag.

void trading(int clientSocket);
...
int main(){
  ...
  // listens on the serverSocket and throws an error if -1.
  ...
  while(serverShutdownFlag != 1){
    ...
    trading(clientSocket);        // runs the trading function which hosts user commands and such.
  }
...
void trading(int client_socket){
  ...
  int clientSocket = client_socket; // passing argument to clientSocket.
  int k;                            // var that stores error value.
